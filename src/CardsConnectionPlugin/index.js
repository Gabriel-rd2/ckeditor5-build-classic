...
export default class CardsConnectionPlugin extends Plugin {
  ...
  init() {
    ...
    console.log("CardsConnectionPlugin was initialized");
  }
  ...
  _defineConverters() {
    ...
    function upcastConverter(event, data, conversionApi) {
      ...
      const cardtitle =
        card !== undefined ? card.title : viewAnchor.getAttribute("cardtitle");
      const cardlink =
        card !== undefined ? card.link : viewAnchor.getAttribute("href");
      ...
      if (card === undefined && cardid !== -1) {
        modelElement = conversionApi.writer.createElement("cardconnection", {
          carddeleted: true,
        });
      } else {
        modelElement = conversionApi.writer.createElement("cardconnection", {
    ...
    conversion
      .for("editingDowncast")
      .add((dispatcher) =>
        dispatcher.on("insert:cardconnection", downcastConverter("editing"))
      );

    conversion
      .for("dataDowncast")
      .add((dispatcher) =>
        dispatcher.on("insert:cardconnection", downcastConverter("data"))
      );

    // Conversão do modelo para as view de edição
    function downcastConverter(pipeline) {
      return (event, data, conversionApi) => {
        const viewElement = createViewElement(data, conversionApi, pipeline);
    ...
    function createViewElement(data, conversionApi, pipeline) {
      ...
      if (carddeleted === undefined) {
        ...
        viewElement = conversionApi.writer.createRawElement(
          ...
          function (domElement) {
            const innerText = config.get("cardconnections.cardDeletedMessage");
